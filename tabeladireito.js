...
const TABLE_NAME = "direito"; // Nome da tabela "direito"
...
function montarTabela(data) {
  ...
  // Cabeçalhos dinâmicos, removendo a coluna 'id'
  const columns = Object.keys(data[0]).filter(col => col !== "id" && col !== "created_at");
  tableHead.innerHTML = columns
    .map(col => `<th data-column="${col}">${col === "direito" ? "Direito" : col}</th>`)
    .join("");
  ...
  // Adicionar evento para salvar ao perder o foco nas células editáveis
